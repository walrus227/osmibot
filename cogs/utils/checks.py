...
try:
    with open('permissions.json', 'r') as p:
        ...
        botowner = permissions["ownerid"]
        serverid = permissions["serverid"]
        modrole = permissions["modrole"]
        adminrole = permissions["adminrole"]
except Exception:
    print('Permissions not configured! Run option five.')

def is_owner_check(message):
    return message.author.id == botowner
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name in (modrole, adminrole), **perms)

    return commands.check(predicate)

def admin_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name == adminrole, **perms)
